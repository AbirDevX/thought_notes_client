...
import { singUpValidation } from "../validation/validation";
...
function SignUp() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const errors = singUpValidation(fromData);
    try {
      if (Object.keys(errors).length > 0) {
        setErrors(errors);
        return;
      }
      const { status } = await signUpHelper(fromData);
      if (status !== 201) throw new Error("Bad requested..!");
      setFromData({
        name: "",
        email: "",
        mobile: "",
